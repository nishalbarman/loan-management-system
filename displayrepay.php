...
<body onload="populateLoan(<?php echo $username; ?>)">
        ...
        <script>
            function selectionFunction(target) {
                ...
                fetch("getRepayTable.php?table=" + value).then(res => res.json()).then(data => {
                    console.log(data);
                    // document.getElementById("repayListTable").style.display = "";
            ...
            function populateLoan(target) {
                let value = target;
                fetch("admin/getLoans.php?member=" + value).then(res => res.json()).then(data => {
